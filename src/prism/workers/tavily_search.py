"""
Tavily Search agent using Claude with Tavily MCP tools.

Uses Claude CLI configured with Tavily MCP server
for search and extraction capabilities.
"""
...
class TavilySearchAgent(Agent):
    ...
    Uses tavily_search and tavily_extract via MCP.
    ...
    async def execute(
        ...
        system_prompt = get_registry().build_system_prompt("tavily")
        ...
        if not result.success:
            return AgentResult.from_error(
                error=result.error_message or "Tavily search execution failed",
